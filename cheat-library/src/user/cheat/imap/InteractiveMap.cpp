...
#undef APPLY_SCENE_OFFSETS
    ...
    void InteractiveMap::DrawPoints()
    {
        ...
        //LOG_TRACE("sceneID=%d", sceneID);
        if (m_ScenesData.count(sceneID) == 0) {
            LOG_TRACE("%d ScenesData is empty", sceneID);
            return;
        }
        ...
        for (auto& [labelID, label] : labels)
        {
            ...
            for (auto& [pointID, point] : label.points)
            {
                ...
                //LOG_TRACE("%d %s", sceneID, label.clearName.c_str());
                //ImGui::PushID(&point);
    ...
    static float GetMinimapScale()
    {
        ...
        auto& fileds = _monoMiniMap->fields.context->fields;
        //LOG_TRACE("%d Scale %.2f / %.2f", fileds._sceneID, fileds._miniMapScale, fileds._defaultMiniMapScale);
